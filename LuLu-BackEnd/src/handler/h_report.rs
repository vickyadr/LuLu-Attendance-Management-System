...
pub async fn report_today(pool: web::Data<AppState>, bearer: Option<ReqData<String>>) -> impl Responder {
    ...
    match sqlx::query_as::<_, ScheduleHelper>(r#"SELECT * FROM schedules INNER JOIN shifts ON shifts.shift_id = schedules.schedule_shift_id"#)
    .fetch_all(&pool.db)
    .await {
        Ok(res)=>{
            for d in res
    ...
    match sqlx::query_as::<_, ReportsHelper>(r#"SELECT device_timezone, device_name, device_location, enroll_device_sn, enroll_type, enroll_status, enroll_time, employee_id, employee_fname, employee_lname, employee_address, employee_departement, employee_status, employee_schedule_id, schedule_type
		...
		INNER JOIN schedules ON schedules.schedule_id = employee.employee_schedule_id) ORDER BY enroll_time ASC"#)
    .fetch_all(&pool.db)
    .await
    {
        Ok(datas) => {
            let mut start_date : i64 = ts_at_zero(1728097795, 0);
            let end_date : i64 = ts_at_zero(1754179200, 0);
