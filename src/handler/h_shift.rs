...
pub async fn shift_add(pool: web::Data<AppState>, body: web::Json<ReceiverShift>, bearer: Option<ReqData<String>>) -> impl Responder {
    ...
    match sqlx::query(r#"INSERT INTO shifts (shift_name, shift_start_time, shift_end_time, shift_start_enroll, shift_end_enroll, shift_passday) VALUES ($1, $2, $3, $4, $5, $6)"#)
        ...
        {
            Ok(_data)=>{
                match sqlx::query_as::<_, Shifts>(r#"SELECT * FROM shifts ORDER BY shift_id DESC"#)
                    .fetch_one(&pool.db)
                    .await
                    {
...
pub async fn shift_edit(pool: web::Data<AppState>, body: web::Json<ReceiverShift>, bearer: Option<ReqData<String>>) -> impl Responder {
    ...
    match sqlx::query(r#"UPDATE shifts SET shift_name=$1, shift_start_time=$2, shift_end_time=$3, shift_start_enroll=$4, shift_end_enroll=$5, shift_passday=$6 WHERE shift_id=$7"#)
                .bind(body.name.clone())
                .bind(body.start_time)
                .bind(body.end_time)
                .bind(start_enroll)
                ...
                .await
        {
            Ok(_data)=>{
                ...
                match sqlx::query_as::<_, Shifts>(r#"SELECT * FROM shifts WHERE shift_id = $1"#)
                    .bind(body.id)
                    .fetch_one(&pool.db)
                    .await
                    {
